...
func _process(delta: float) -> void:
	...
	cell_hovered = Grid2D.GridData[hovered_grid_pos]
...
func add_blocked_cells_for_pathfinder():	
	for i:Vector2i in Grid2D.GridData.keys():
		var tile : TileData = get_cell_tile_data(i)
...
func set_cell_data(unit:Character):
	...
	Grid2D.GridData[gridPos].UnitOccupying = unit
...
func GetRandomGridCell() -> Vector2i:
	...
	var cell_data : TileData = get_cell_tile_data(randCell)
	var cell_is_blocked : bool = cell_data.get_custom_data("Block")
	if Grid2D.GridData[randCell].UnitOccupying or cell_is_blocked:
